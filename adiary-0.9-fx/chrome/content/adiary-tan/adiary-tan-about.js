var adiaryTanStringBundle = {
  _bundle: Components.classes["@mozilla.org/intl/stringbundle;1"]
             ...
             .createBundle("chrome://adiary-tan/locale/adiary-tan.properties"),
...
var adiaryTanAbout = {
  changeStyle: function(aBox, aBalloon, aLabel1, aLabel2, aLabel3,
                        aType, aArgs) {
    var propertyName = "adiaryTanAbout.type" + aType;
    var backgroundImageValue = adiaryTanStringBundle
                                 .getString(propertyName + ".backgroundImage");
    var labelValue;

    if (aArgs) {
      aArgs = [].slice.call(arguments, 6);
      aArgs.unshift(propertyName + ".label");
      labelValue = adiaryTanStringBundle.getString
                                        .apply(adiaryTanStringBundle, aArgs);
    } else {
    ...
    if (box && balloon) {
      if (box.getAttribute("adiaryTanClicked") == "true") {
        ...
        if (Math.random() > 0.6) {
          ...
          this.changeStyle(box, balloon, label1, label2, label3, 1,
                           nowHour, nowMinute);
        }
  ...
  openWebsite: function(aURL) {
    document.getElementById("aboutDialog").acceptDialog();
    window.opener.getBrowser().selectedTab = window.opener.getBrowser()
                                                          .addTab(aURL);
  }
};
