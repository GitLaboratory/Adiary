...
var adiaryTanStatus = {
	init : function() {
		...
		gBrowser.addProgressListener(adiaryTanProgressListener, Components.interfaces.nsIWebProgress.NOTIFY_STATE_DOCUMENT);
	},
	...
	showStatus : function() {
		...
		if (panel) {
			...
			for (var i = 0; i < metaCollection.length; i++) {
				if (metaCollection[i].name.search(/^generator$/) != -1 && metaCollection[i].content.search(/^adiary/) != -1) {
					isAdiary = true;
...
window.addEventListener("load", function() {
	adiaryTanStatus.init()
}, false);
window.addEventListener("unload", function() {
	adiaryTanStatus.uninit()
}, false);
